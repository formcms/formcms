...
public class AssetApiTest(AppFactory factory)
{
    ...
    public async Task GetEntity()
    {
        ...
        Assert.NotNull(
            entity.Attributes.FirstOrDefault(x => x.Field == nameof(Asset.LinkCount).Camelize())
        );
    ...
    public async Task TestMeta()
    {
        ...
        {
            Title = "Title",
            Metadata = new Dictionary<string, object> { { "color", "yellow" } },
        };
    ...
    private async Task<AssetLink?> EntityHasAsset(
        string assetName,
        string entityName,
        long recordId
    )
    {
        ...
        var asset = await factory.AssetApi.Single(id).Ok();
        return asset.Links?.FirstOrDefault(x =>
            x.EntityName == entityName && x.RecordId == recordId
        );
    }
    ...
    public async Task AddDataWithFileAsset()
    {
        ...
        var imagesPath = await factory
            .AssetApi.AddAsset([(img1, Create2048TileImage()), (img2, Create2048TileImage())])
            .Ok();
        ...
        imagesPath = await factory.AssetApi.AddAsset([(updateImage, Create2048TileImage())]).Ok();
