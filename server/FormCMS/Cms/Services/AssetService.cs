...
) : IAssetService
{
    ...
    private async Task<Asset> Single(
        SqlKata.Query query,
        bool loadLink,
    ...
    )
    {
        ...
        var res = await hookRegistry.AssetPreList.Trigger(
            provider,
            new AssetPreListArgs([.. filters])
        );
    ...
    public async Task<string[]> Add(IFormFile[] files, CancellationToken ct)
    {
        foreach (var formFile in files)
        {
            if (formFile.Length == 0)
                throw new ResultException($"File [{formFile.FileName}] is empty");
        }

        files = files.Select(resizer.CompressImage).ToArray();
        var dir = DateTime.Now.ToString("yyyy-MM");
        var pairs = files
            .Select(x => (Path.Join(dir, UniqNameOmitYearAndMonth(x.FileName)), x))
            .ToArray();
        Asset? asset = null;
    ...
    {
        if (file.Length == 0)
            throw new ResultException($"File [{file.FileName}] is empty");
        ...
        try
        {
            var updateQuery = Assets.UpdateFile(
                asset.Id,
                file.FileName,
                file.Length,
                file.ContentType
    ...
    public async Task UpdateAssetsLinks(
        Record[] oldLinks,
        string[] newAssets,
        string entityName,
        long id,
    ...
    )
    {
        ...
        await executor.BatchInsert(
            AssetLinks.TableName,
            AssetLinks.ToInsertRecords(entityName, id, toAdd)
        );

        if (toDel.Length > 0)
        {
            await executor.Exec(
                AssetLinks.DeleteByEntityAndRecordId(entityName, id, toDel),
                false,
                ct
            );
        }
    ...
    private async Task<Record[]> EnsurePathTracked(
        string[] assetPaths,
        Record[] assetRecords,
        CancellationToken ct
    )
    {
        var set = assetRecords.Select(x => x.StrOrEmpty(nameof(Asset.Path).Camelize())).ToHashSet();
        var list = new List<Asset>();
        foreach (var s in assetPaths)
        {
            if (set.Contains(s) || !Assets.IsValidPath(s))
                continue;
    ...
    private async Task LoadLinkCount(Record[] items, CancellationToken ct)
    {
        ...
        var dict = await executor.LoadDict(
            ...
            nameof(AssetLink.AssetId).Camelize(),
            nameof(Asset.LinkCount).Camelize(),
            ct
        );
